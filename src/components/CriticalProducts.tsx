      ...
      try {
        ...
        const enriched: DisplayProduct[] = [
          ...critical.map((product) => ({
            ...
            location:
              product.location || translate("Sin ubicación", "No location"),
        ...
        const message =
          err instanceof Error
            ? err.message
            : translate(
                "No se pudieron obtener los productos críticos.",
                "Unable to fetch critical products.",
              );
        setError(message);
      } finally {
        setLoading(false);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5 sm:mb-6">
        ...
        >
          <h3 className="font-semibold text-gray-900 dark:text-white mb-1 text-base sm:text-lg">
            {translate("Productos críticos", "Critical products")}
          </h3>
          <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
            {translate(
              "Niveles de stock que requieren atención inmediata",
              "Stock levels needing immediate attention",
            )}
          </p>
        </motion.div>
      </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1">
                      ...
                      <div
                        className={`bg-gradient-to-r ${status.color} text-white text-xs px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg font-semibold shadow-sm`}
                      >
                        {status.label}
                      </div>
                    </div>
                  ...
                  <div className="text-right flex-shrink-0">
                    <div
                      className={`text-xl sm:text-2xl font-bold ${status.textColor}`}
                    >
                      {formatNumber(product.stock)}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-gray-400 font-medium">
                      {translate("de", "of")} {formatNumber(product.minStock)}
                    </div>
                  </div>
                </motion.div>
              );
